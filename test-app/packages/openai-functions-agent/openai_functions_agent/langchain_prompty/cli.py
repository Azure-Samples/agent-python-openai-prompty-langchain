...
from .processors import ToolCall
...
@click.option("--output", "-o", default=".runs")
@click.option("--chat", "-c", is_flag=True)
@click.version_option()
@trace
def run(source, verbose, output, chat):
    #initialize_tracer(output_dir=normalize_path(output, create_dir=True))
